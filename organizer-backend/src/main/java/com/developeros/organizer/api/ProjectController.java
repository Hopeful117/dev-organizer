...
public class ProjectController {
    ...
    @PutMapping("/{id}/devlog")
    public ResponseEntity<ProjectResponse> linkDevlog(
            @PathVariable UUID id, @Valid @RequestBody ProjectDevlogLinkRequest request) {
        return ResponseEntity.ok(ProjectResponse.from(
                service.linkDevlogProject(id, request.devlogProjectId(), request.devlogProjectSlug())));
    }

    @DeleteMapping("/{id}/devlog")
    public ResponseEntity<ProjectResponse> unlinkDevlog(@PathVariable UUID id) {
        return ResponseEntity.ok(ProjectResponse.from(service.unlinkDevlogProject(id)));
    }
}
